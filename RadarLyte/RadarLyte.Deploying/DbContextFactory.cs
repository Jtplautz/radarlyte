...
public class RadarLyteIdentityContextFactory : IDesignTimeDbContextFactory<RadarLyteIdentityContext> {
    public RadarLyteIdentityContext CreateDbContext(string[] args)
    {
        IConfigurationRoot configuration = new ConfigurationBuilder()
                                            .SetBasePath(Directory.GetCurrentDirectory())
                                            .AddJsonFile("appsettings.json")
        ...
        IServiceCollection services = new ServiceCollection();
        services.AddDbContextPool<RadarLyteIdentityContext>(options =>
        {
            options.EnableSensitiveDataLogging();
            options.ConfigureWarnings(warnings =>
            {
            ...
            options.UseSqlServer(configuration.GetConnectionString("RadarLyteIdentityContextConnection"), sqlOptions =>
            {
                sqlOptions.EnableRetryOnFailure();
                sqlOptions.MigrationsAssembly(typeof(RadarLyteIdentityContext).Assembly.GetName().Name);
            });
        });

        ServiceProvider serviceProvider = services.BuildServiceProvider();
        return serviceProvider.GetService<RadarLyteIdentityContext>();
    }
}