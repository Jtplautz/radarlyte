...
namespace RadarLyte.Extensions;
public static class ViteDevelopmentServerConfiguration {
    ...
    public static void UseViteDevelopmentServer(this ISpaBuilder spa, int? port = null)
    {
        ...
        if (!IsRunning)
        {
            ...
            //var tempPfx = Path.Combine(spaFolder, "RadarLyteWeb.pfx");
            ...
            {
                logger.LogWarning($"node_modules not found , run yarn install...");
                // Install node modules
                var ps = Process.Start(new ProcessStartInfo()
                {
                    FileName = isWindows ? "cmd" : "yarn",
                    Arguments = $"{(isWindows ? "/c yarn " : "")}install",
                    WorkingDirectory = "C:\\Code\\radarlyte\\RadarLyte\\RadarLyte.Web\\InteractiveClient",
                    RedirectStandardError = true,
                    RedirectStandardInput = true,
                    RedirectStandardOutput = true,
                    UseShellExecute = false,
                ...
                ps.WaitForExitAsync();
                logger.LogWarning($"yarn install done.");
            }
